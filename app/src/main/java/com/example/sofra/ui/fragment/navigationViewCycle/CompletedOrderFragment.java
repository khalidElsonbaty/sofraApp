...
public class CompletedOrderFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        myRecyclerView = (RecyclerView) view.findViewById(R.id.Completed_Order_RecyclerView);
        ...
        OnEndless onEndless = new OnEndless(linearLayoutManager, 1) {
            ...
            public void onLoadMore(int current_page) {
                if (current_page <= max) {
                    setCompletedOrderData(current_page);
                }

            }
        };
        myRecyclerView.addOnScrollListener(onEndless);
        recyclerViewAdapter = new MyOrderRecycler(getContext(), orderData, "completed");
        myRecyclerView.setAdapter(recyclerViewAdapter);

        setCompletedOrderData(1);

        return view;
    }

    private void setCompletedOrderData(int page) {
        apiServices.getOrder("HRbqKFSaq5ZpsOKITYoztpFZNylmzL9elnlAThxZSZ52QWqVBIj8Rdq7RhoB",
                "completed", page).enqueue(new Callback<Order>() {
            @Override
